 ...
 * The WebGL canvas is created with `preserveDrawingBuffer`, so we can read it
 * back at any time without a re-render.
 */
...
export function composeStamped(
	...
	if (opts.code) {
		...
		ctx.fillText(opts.code.toUpperCase(), pad + Math.round(14 * scale), pad + Math.round(14 * scale))
		ctx.restore()
	}
