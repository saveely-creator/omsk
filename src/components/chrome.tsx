		...
		<div ref={ref} className="cursor-dot" aria-hidden="true">
			<div
				style={{
					...
					display: "grid",
					placeItems: "center",
					transition: "width 320ms var(--ease-lab), height 320ms var(--ease-lab)",
				}}
			>
				{mode === "emoji" ? (
					<span style={{ fontSize: 18, mixBlendMode: "normal" }}>{CHAT.topEmoji[0]?.e}</span>
				) : label ? (
					<span
						className="micro"
						style={{ color: "var(--clay)", fontSize: 8, letterSpacing: "0.1em", textAlign: "center" }}
					>
						{label}
					</span>
				) : null}
...
export function Footer() {
	...
	return (
		<footer className="above shell" style={{ paddingTop: 48, paddingBottom: 40 }}>
			<div className="hairline" style={{ marginBottom: 24 }} />
			<div className="grid gap-8 md:grid-cols-3">
				<div>
					<div className="micro">цитата дня</div>
					<p className="display" style={{ fontSize: "1.5rem", lineHeight: 1.2, marginTop: 8 }}>
						«{today.quote}»
					</p>
					<Link
...
export function Toggles() {
	const { sound, acid, vhs, motion } = useUi((s) => ({
		sound: s.sound,
		acid: s.acid,
		vhs: s.vhs,
		motion: s.motion,
	...
	return (
		<div className="flex flex-wrap gap-2">
			...
			<button type="button" className="chip" aria-pressed={acid} onClick={toggleAcid}>
				ацид
			</button>
			<button type="button" className="chip" aria-pressed={vhs} onClick={toggleVhs}>
				vhs
			</button>
			<button
				type="button"
...
function Achievements() {
	...
	return (
		<div className="micro" style={{ color: visitedAll(visited) ? "var(--accent)" : undefined }}>
...
export function DragCarousel({
	...
	const state = useRef({ x: 0, target: 0, min: 0, down: false, startX: 0, startTarget: 0, v: 0 })
	const off = useMotionOff()
	const setCursor = useUi((s) => s.setCursor)
	...
	useEffect(() => {
		const measure = () => {
			...
			state.current.min = Math.min(0, viewport.current.clientWidth - track.current.scrollWidth)
		}
		measure()
	...
	const nudge = (dir: 1 | -1) => {
		...
		state.current.target = Math.max(state.current.min, Math.min(0, state.current.target - dir * w))
	}
		...
		<div className={className}>
			...
			<div className="mt-6 flex items-center gap-4">
				...
				<div className="flex gap-2">
					<button type="button" className="chip" onClick={() => nudge(-1)} aria-label="назад">
						←
					</button>
					<button type="button" className="chip" onClick={() => nudge(1)} aria-label="вперёд">
						→
					</button>
				</div>
			</div>
...
export function ScrollHint({ text = "Скролльте" }: { text?: string }) {
	return (
		<div
			...
			aria-hidden="true"
		>
			<span style={{ display: "inline-block", animation: "none" }}>↓</span> {text}
		</div>
	)
}
