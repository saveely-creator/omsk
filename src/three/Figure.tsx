...
export function Figure({
	...
	useFrame((state) => {
		...
		if (headRef.current) {
			headRef.current.rotation.y = pose.head[1] + Math.sin(t * 0.33) * 0.05 * amp
			headRef.current.rotation.x = pose.head[0] + Math.sin(t * 0.51) * 0.02 * amp
		}
	...
	const legs = ([-1, 1] as const).map((side) => {
		...
		const long = items.bottom.item === "jeans" || items.bottom.item === "sweats"
		...
		return (
			...
			>
				...
				<group position={[0, -L.thigh * prop.limb, 0]} rotation={[leg.knee, 0, 0]}>
					<mesh position={[0, -L.shin / 2, 0]}>
						<capsuleGeometry args={[r * 0.86, L.shin * prop.limb, cap(4), cap(12)]} />
						<Surface color={body} opacity={opacity} />
					</mesh>
	...
	const arms = ([-1, 1] as const).map((side) => {
		...
		return (
			<group
				key={"arm" + side}
				position={[side * prop.shoulderX * 1.16, Y.shoulder, 0]}
				rotation={arm.shoulder}
			>
				<mesh position={[0, -L.upperArm / 2, 0]}>
					<capsuleGeometry args={[r, L.upperArm * prop.limb, cap(4), cap(12)]} />
					<Surface color={body} opacity={opacity} />
				</mesh>

				<group position={[0, -L.upperArm * prop.limb, 0]} rotation={[arm.elbow, 0, 0]}>
					<mesh position={[0, -L.forearm / 2, 0]}>
						<capsuleGeometry args={[r * 0.86, L.forearm * prop.limb, cap(4), cap(12)]} />
						<Surface color={body} opacity={opacity} />
					</mesh>
		...
		<group {...group}>
			<group ref={root} scale={height / 1.74}>
				<group position={[0, pose.hipY, 0]}>
					...
					{/* hips */}
					<mesh position={[0, Y.hip + 0.02, 0]} scale={[prop.build, 0.8, prop.build * 0.82]}>
					...
					<group ref={torso} position={[0, Y.chest, 0]} rotation={pose.spine}>
						...
						<group position={[0, 0.02, -0.05]}>
							<BackItem item={items.back.item} color={color.back} width={prop.build} />
						</group>
						...
						<mesh position={[0, 0.15, 0]}>
							<cylinderGeometry args={[0.03, 0.034, prop.neckLen + 0.03, cap(12)]} />
							<Surface color={body} opacity={opacity} />
						</mesh>

						<group ref={headRef} position={[0, Y.head - Y.chest, 0]} rotation={pose.head}>
							<mesh scale={[prop.headScale, prop.headScale * 1.08, prop.headScale * 0.98]}>
								<sphereGeometry args={[0.085, cap(24), cap(18)]} />
								<Surface color={body} opacity={opacity} />
							</mesh>

							{/* generalised features, like a rough 3D scan */}
							<mesh position={[0, -0.004, 0.078]} rotation={[Math.PI / 2, 0, 0]}>
							...
							items.head.item === "horns" ? (
								<mesh position={[0, hair.y, -0.004]} scale={hair.scale as unknown as THREE.Vector3Tuple}>
									<sphereGeometry
										args={[0.086, cap(20), cap(14), 0, Math.PI * 2, 0, Math.PI * hair.phi]}
									/>
									<Surface color={body} opacity={opacity} />
								</mesh>
							) : null}
