...
import { BASE_OUTFIT, PRESETS, type Outfit, type PresetId, type Slot } from "@/data/wardrobe"
...
export const useOutfitStore = create<OutfitState>()(
	persist(
		(set) => ({
			...
			applyPreset: (slug, preset) =>
				set((s) => {
					...
					return {
						overrides: {
							...s.overrides,
							[slug]: { ...(s.overrides[slug] ?? {}), ...PRESETS[preset].patch },
						},
					}
				}),
			...
			shuffleArrangement: () => set((s) => ({ arrangement: s.arrangement + 1 })),
		}),
...
export function resolveOutfit(slug: string, overrides: Partial<Outfit> | undefined): Outfit {
